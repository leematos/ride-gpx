...
// center with a paler amber ring. The 3D marker is an actual mesh
// (app/assets/rider-dot.glb, see RIDER_DOT_MODEL_URL below) with those
// colors baked into its two materials — RIDER_DOT_COLOR here only feeds the
// minimap's 2D marker icon and the Polyline3DElement fallback for browsers
// without Model3DElement.
...
const RIDER_DOT_MODEL_URL = new URL("assets/rider-dot.glb", import.meta.url);
...
function renderRiderDot(point) {
  ...
  // A Model3DElement (an actual mesh: app/assets/rider-dot.glb, generated by
  // scripts/generate_rider_dot_model.py), not a Polygon3DElement: a filled
  // ground polygon is meant for static terrain-draped areas, and
  // re-tessellating one every frame as the rider moves produced two separate
  // failures confirmed in a real browser — the fill rendering solid black at
  // ordinary follow-camera distances (independent of winding, altitude, and
  // extrusion, all tried), and a faceted/streaky look from the constant
  // re-triangulation. `orientation.tilt: 90` corrects for the model's local
  // up axis not mapping to world-up by default — without it the disc stands
  // on its edge instead of lying flat (confirmed by testing; not asserted by
  // the Maps 3D docs anywhere we could find).
  if (Model3DElement) {
